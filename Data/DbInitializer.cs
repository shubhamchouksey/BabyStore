...
namespace BabyStore.Data
{
    public class DbInitializer 
    {
        ...
        {
            using (var context = new BabyStoreContext(
                serviceProvider.GetRequiredService<DbContextOptions<BabyStoreContext>>()))
            {
                ...
                var adminID = await EnsureUser(serviceProvider, password, name);
                await EnsureRole(serviceProvider, adminID, roleName);

                //Create User Role
                const string userRoleName = "Users";
                if (!await serviceProvider.GetService<RoleManager<IdentityRole>>().RoleExistsAsync(userRoleName))
                {
                    var roleresult = serviceProvider.GetService<RoleManager<IdentityRole>>().CreateAsync(new IdentityRole(userRoleName));
                }
        ...
        public static void SeedDb(BabyStoreContext context)
        {
           ...
           var images = new List<ProductImage>
            ...
            var orders = new List<Order>
            {
                new Order
                {
                    DeliveryAddress = new Address { AddressLine1 = "1 Some Street", Town = "Town1", County = "County", Postcode = "PostCode" },
                    TotalPrice = 4.99M,
